...
const TeamRow = ({ team, score, winner, testId }) => (
  <div data-testid={testId} className={`flex items-center gap-2 px-2 py-1.5 ${winner ? "bg-[#D8CA82]/10" : ""}`}>
    ...
    <span className={`text-xs truncate flex-1 ${team ? "text-white" : "text-zinc-400 italic"} ${winner ? "font-bold text-[#D8CA82]" : ""}`}>{team?.name || "TBD"}</span>
...
export const MatchCard = ({ match: m, tournament, compact = false, onOpen }) => (
  <button type="button" data-testid={`match-card-${m.id}`} onClick={() => onOpen(m)} className={`card-elysium hoverable w-full text-left ${compact ? "w-56" : ""} ${m.status === "disputed" ? "border-red-500/50" : m.status === "ready" ? "border-[#D8CA82]/40" : ""}`}>
    <div className="flex items-center justify-between px-2 pt-1.5"><span className="text-[10px] uppercase tracking-wider text-zinc-400">R{m.round} · M{m.index + 1}{m.isBye && " · BYE"}</span><StatusBadge status={m.status} testId={`match-status-${m.id}`} /></div>
...
export const MatchDialog = ({ match: m, tournament, onClose }) => {
  ...
  const [rep, setRep] = useState({ a: "", b: "" }); const [org, setOrg] = useState({ a: "", b: "" });
  const [proof, setProof] = useState(null); const [reason, setReason] = useState(""); const [busy, setBusy] = useState(false);
  if (!m) return null;
  ...
  const openChat = () => run(async () => { const cid = m.conversationId || (await createMatchConversation(m, tournament, profile)); nav(`/messages/${cid}`); });
    ...
    <Dialog open={!!m} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="bg-[#181818] border-white/10 rounded-none max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="match-dialog">
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 py-2">
          ...
          <span className="order-2 font-display text-zinc-400">VS</span>
        </div>
          ...
          <div className="space-y-2" data-testid="match-reports">
            ...
            {Object.entries(m.reports).map(([teamId, r]) => (
              <div key={teamId} className="border border-white/10 p-3 flex flex-wrap items-center gap-3 text-xs">
                ...
                <span className="text-zinc-400">{formatDate(r.at, true)}</span>
        ...
        {canAct && myTeam && !isOrg && (
          <div className="border border-white/10 p-4 space-y-3" data-testid="match-report-form">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label={m.teamA.name}><input data-testid="report-score-a" type="number" min={0} className="input-elysium" value={rep.a} onChange={(e) => setRep({ ...rep, a: e.target.value })} /></Field>
              <Field label={m.teamB.name}><input data-testid="report-score-b" type="number" min={0} className="input-elysium" value={rep.b} onChange={(e) => setRep({ ...rep, b: e.target.value })} /></Field>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button data-testid="report-submit" disabled={busy || rep.a === "" || rep.b === ""} onClick={() => run(() => reportMatch(m, myTeam.id, Number(rep.a), Number(rep.b), proof, user.uid), t("score_reported"))} className="btn-gold text-xs ml-auto"><Check className="h-4 w-4" />{t("submit_score")}</button>
            </div>
        ...
        {isOrg && m.teamA && m.teamB && (
          <div className="border border-[#D8CA82]/30 bg-[#1C1910] p-4 space-y-3" data-testid="match-organizer-panel">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label={m.teamA.name}><input data-testid="org-score-a" type="number" min={0} className="input-elysium" value={org.a} onChange={(e) => setOrg({ ...org, a: e.target.value })} /></Field>
              <Field label={m.teamB.name}><input data-testid="org-score-b" type="number" min={0} className="input-elysium" value={org.b} onChange={(e) => setOrg({ ...org, b: e.target.value })} /></Field>
            </div>
            <button data-testid="org-resolve-submit" disabled={busy || org.a === "" || org.b === "" || org.a === org.b} onClick={() => run(() => resolveMatch(m, Number(org.a), Number(org.b), user.uid), t("match_resolved"))} className="btn-gold text-xs w-full"><Gavel className="h-4 w-4" />{t("set_result")}</button>
          </div>
        )}
        {m.status === "done" && <p className="text-xs text-zinc-400 text-center">{t("match_done_note")}{m.resolvedBy && m.resolvedBy !== "auto" && ` · ${t("resolved_by_organizer")}`}</p>}
