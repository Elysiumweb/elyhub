...
const Section = ({ title, to, children, icon: Icon, seeAll }) => (
  <section>
    <div className="flex items-center justify-between mb-3">
      <h2 className="section-title mb-0"><Icon className="h-3.5 w-3.5" aria-hidden="true" />{title}</h2>
      <Link to={to} className="text-xs text-[#D8CA82] hover:underline inline-flex items-center gap-1">{seeAll}<ArrowRight className="h-3 w-3" aria-hidden="true" /></Link>
    </div>
    {children}
  </section>
);
...
export default function Home() {
  ...
  return (
    <div className="space-y-12">
      <Seo description={t("home_seo")} jsonLd={{ "@type": "WebSite", name: "ElyHub", url: window.location.origin }} />
      <section className="relative overflow-hidden border border-[#D8CA82]/25 bg-[#141414] p-8 sm:p-12">
        <img src="/brand/accent-blade.png" alt="" aria-hidden="true" loading="lazy" width="420" height="420" className="absolute -right-16 -top-16 w-[420px] opacity-30 pointer-events-none hidden md:block" />
        ...
        <div className="relative mt-10 grid grid-cols-2 sm:grid-cols-4 gap-px bg-white/10 border border-white/10 max-w-2xl">
          {[[teams.data.length, "nav_teams", Users], [scrims.data.filter((s) => s.status === "open").length, "open_scrims", Swords], [tournaments.data.length, "nav_tournaments", Trophy], [offers.data.filter((o) => o.status === "open").length, "open_offers", Briefcase]].map(([n, k, I]) => (
            <div key={k} className="bg-[#111111] p-4" data-testid={`stat-${k}`}>
              ...
              <div className="text-[10px] uppercase tracking-widest text-zinc-400">{t(k)}</div>
            </div>
          ))}
        </div>
      </section>

      <nav aria-label={t("games")} className="flex flex-wrap gap-2" data-testid="home-game-hubs">
        {games.filter((g) => g.status === "validated").map((g) => <Link key={g.id} to={`/g/${g.slug}`} data-testid={`home-hub-${g.slug}`} className="badge px-3 py-1.5 text-xs hover:-translate-y-0.5 transition-transform" style={{ color: g.color, borderColor: `${g.color}66`, backgroundColor: `${g.color}14` }}>{g.name}</Link>)}
      </nav>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <Section title={t("latest_scrims")} to="/scrims" icon={Swords} seeAll={t("see_all_scrims")}>
        ...
        <Section title={t("upcoming_tournaments")} to="/tournaments" icon={Trophy} seeAll={t("see_all_tournaments")}>
        ...
        <Section title={t("featured_teams")} to="/teams" icon={Users} seeAll={t("see_all_teams")}>
        ...
        <Section title={t("lft_title")} to="/players?tab=lft" icon={UserSearch} seeAll={t("see_all_lft")}>
        ...
        <Section title={t("open_offers")} to="/teams?tab=offers" icon={Briefcase} seeAll={t("see_all_offers")}>
