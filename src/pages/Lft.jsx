...
export const LftCard = ({ lft }) => {
  ...
  const official = isOfficialItem(lft);
  return (
    <div data-testid={`lft-card-${lft.id}`} className={`card-elysium hoverable relative p-4 ${official ? "card-official" : ""}`}>
      <span className="absolute left-0 top-0 h-full w-[3px]" style={{ backgroundColor: official ? "#D8CA82" : g.color }} />
